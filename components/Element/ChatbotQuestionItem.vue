<template>
  <div>
    <div v-if="editMode" class="p-4 sm:p-6 bg-slate-900 text-white shadow-md rounded">
      <FormChatbotQuestion
        :item="item"
        @close="
          editMode = false;
    ...
    <div v-else>
      ...
      <div class="pl-3.5 pr-2.5 flex justify-between gap-4 cursor-pointer bg-white hover:bg-sky-50 text-sky-800" @click="expanded = !expanded">
        <div class="my-3" :class="{ 'text-sm line-clamp-1': !expanded }" v-html="highlight(body, searchTerm) || '&nbsp;'"></div>

        <button v-if="body" class="text-sky-600 leading-none">
          <Icon v-if="!expanded" name="ph:caret-down-bold" class="text-4xl" />
          <Icon v-else name="ph:caret-up-bold" class="text-4xl" />
          <span class="sr-only">{{ $t('EDIT') }}</span>
        </button>
      </div>
    </div>
