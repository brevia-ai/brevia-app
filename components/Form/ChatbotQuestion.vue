<template>
  <form class="flex flex-col space-y-6" @submit.prevent="save">
    <UIXInput
      v-model.trim="title"
      :label="item.custom_id ? '' : $t('NEW_QUESTION')"
    ...
    <div class="flex justify-end gap-4">
      <button v-if="item.custom_id" class="mr-auto button button-danger uppercase" :class="{ 'is-loading': isDeleting }" @click.prevent="deleteQuestion">
      ...
      <button type="submit" class="px-8 button button-primary uppercase" :class="{ 'is-loading': isSaving }" :disabled="!title">
...
const readMetadata = async (docId: string) => {
  try {
    const response = await $fetch(`/api/brevia/index/${collectionUuid}/${docId}`, { method: 'GET' });
