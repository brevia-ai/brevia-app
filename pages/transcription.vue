        ...
        <div class="space-y-6 sm:space-y-8">
            ...
            <div class="text-center sm:text-left">
                <button class="w-full sm:w-auto px-8 py-2 sm:py-4 button"
                    ...
                    :disabled="!file || isBusy" @click="submit">{{ $t('UPLOAD_AND_TRANSCRIBE_FILE') }}</button>
                <button class="mt-4 sm:ml-6 sm:mt-0 px-8 py-2 sm:py-4 bg-red-900 hover:bg-red-700 button"
                    :disabled="!file || isBusy" @click="reset" v-if="transcription || error">{{ $t('RESET') }}</button>
            </div>

            <hr class="border-neutral-300" v-if="transcription">
            <div class="space-y-4" v-if="transcription">
                <h2 class="text-xl leading-tight">{{ $t('TRANSCRIPTION_OF_FILE') }} <span class="block md:inline font-bold">{{ file.name }}</span></h2>
